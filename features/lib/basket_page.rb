...
class BasketPage
  ...
  link(:proceed_to_checkout, class: %w(button btn btn-default standard-checkout button-medium))

  def no_of_products_in_basket
    basket_contents_element.trs(id: /product_(\S+)/).count
  end

  def products_total
    item_total.delete('$').to_f.round(2)
  end

  def shipping_total
    shipping_cost.delete('$').to_f.round(2)
  end

  def total_price_without_tax
    net_price.delete('$').to_f.round(2)
  end
end
